...
**1,435 points**, OJH score version 2, provisional

![OJH score 1,435](score-freeciv.svg)
...
- the Frame rate run did not finish cleanly, so its figures may be partial
- scored on 80% of the weight; not reported: Frame rate, Smoothness

## Parts

| Part | Measured by | Weight | Measured | On the reference CPU | Worth 1,000 points | Points |
|---|---|---|---|---|---|---|
| Turn throughput | ojh tpm | 22% | 711.8 player-turns/min | 416.3 player-turns/min | 2,000 player-turns/min | 273 |
| World throughput | ojh tpm | 13% | 230,630 region-turns/min | 134,895 region-turns/min | 200,000 region-turns/min | 744 |
| Late-game pace | ojh tpm | 8% | 0.15 | same | 0.50 | 372 |
| Steadiness | ojh tpm | 5% | 0.57 | same | 0.50 | 1,101 |
| Start-up | ojh tpm | 5% | 0.497 s | 0.850 s | 10.0 s | 3,675 |
| CPU per player-turn | ojh tpm | 6% | 0.0831 s | 0.142 s | 0.0100 s | 98 |
| Memory | ojh tpm | 8% | 58.6 MiB | same | 2.00 GiB | 5,169 |
| Frame rate | ojh fps | 12% | not measured | n/a | 60 fps | n/a |
| Smoothness | ojh fps | 8% | not measured | n/a | 30 fps | n/a |
| Data per turn | ojh net | 9% | 39.3 KiB | same | 256 KiB | 2,909 |
| Turn delivery | ojh net | 4% | 0.0559 s | 0.0957 s | 0.100 s | 1,032 |
...
- **Turn speed**: gaps between Freeciv's per-turn "End/start-turn server/ai activities" log lines (249 turns timed of 250 markers)
- **Frame rate**: not measured: Freeciv's client redraws only when something on the map changes, so it has no steady frame rate to time
- **Network**: freeciv-server with 8 AI players and 2 freeciv-gtk4 clients on this machine, each attached as a global observer, every byte through OJH's loopback relay; a turn ends at the server's End/start-turn log line
- **Footprint**: install is the Freeciv folder the server runs from; the save is the largest of Freeciv's own per-turn saves over 20 turns, in its default compression; load time is freeciv-server started on that save minus the same server started empty; Freeciv has no command to time a save, so save time is n/a
- **Turn speed settings**: 250 turns asked for, 249 timed, seed 20260914, 8 players chosen by OJH
- **Machine**: Apple M1 Pro, macOS 26.3 (25D125)
- **CPU reference score**: 1,710 rounds/s on one core; CPU-bound figures were put on OJH's reference CPU (1,000 rounds/s) with a factor of 0.585
...
- **Total**: the weighted mean of the parts the results have. Coverage is how much of the weight that was, here 80%; a part a game was not measured for is left out, never counted as zero.
