...
**1,039 points**, OJH score version 2

![OJH score 1,039](score-gd5.svg)
...
|---|---|---|---|---|---|---|
| Turn throughput | ojh tpm | 22% | 2,612 player-turns/min | 1,533 player-turns/min | 2,000 player-turns/min | 821 |
| World throughput | ojh tpm | 13% | 67,600 region-turns/min | 39,695 region-turns/min | 200,000 region-turns/min | 261 |
| Late-game pace | ojh tpm | 8% | 1.45 | 1.00 (capped) | 0.50 | 1,585 |
| Steadiness | ojh tpm | 5% | 0.61 | same | 0.50 | 1,146 |
| Start-up | ojh tpm | 5% | 1.109 s | 1.889 s | 10.0 s | 2,654 |
| CPU per player-turn | ojh tpm | 6% | 0.0241 s | 0.0411 s | 0.0100 s | 314 |
| Memory | ojh tpm | 8% | 1.02 GiB | same | 2.00 GiB | 1,568 |
| Frame rate | ojh fps | 12% | 162.6 fps | same | 60 fps | 1,891 |
| Smoothness | ojh fps | 8% | 44.02 fps | same | 30 fps | 1,303 |
| Data per turn | ojh net | 9% | 18.1 MiB | same | 256 KiB | 20 |
| Turn delivery | ojh net | 4% | 0.0844 s | 0.144 s | 0.100 s | 762 |
...
- **Turn speed**: OJH's GD5 driver times each turn through GD5's own turn manager, redraw included (250 turns, LLM diplomacy off)
- **Frame rate**: OJH's GD5 driver boots the real game with SDL's dummy video driver and times every update and draw of each scene for 5.00 s after a warm-up, with no frame cap; pygame draws in software, so this is GD5's own drawing cost with no display in the way. The late-game map comes after 20 turns
...
- **Footprint**: install is the Greater Diplomacy 5 folder without .git and caches (Python and its packages not counted); OJH's GD5 driver plays 20 turns, then times GD5's own save_map_data and loading that save into a new Map, and measures the save folder
...
- **CPU reference score**: 1,703 rounds/s on one core; CPU-bound figures were put on OJH's reference CPU (1,000 rounds/s) with a factor of 0.587
