...
**1,566 points**, OJH score version 2

![OJH score 1,566](score-unciv.svg)
...
- scored on 76% of the weight; not reported: Frame rate, Smoothness, Turn delivery

## Parts

| Part | Measured by | Weight | Measured | On the reference CPU | Worth 1,000 points | Points |
|---|---|---|---|---|---|---|
| Turn throughput | ojh tpm | 22% | 5,234 player-turns/min | 3,172 player-turns/min | 2,000 player-turns/min | 1,371 |
| World throughput | ojh tpm | 13% | 471,668 region-turns/min | 285,859 region-turns/min | 200,000 region-turns/min | 1,281 |
| Late-game pace | ojh tpm | 8% | 0.47 | same | 0.50 | 958 |
| Steadiness | ojh tpm | 5% | 0.63 | same | 0.50 | 1,180 |
| Start-up | ojh tpm | 5% | 0.628 s | 1.036 s | 10.0 s | 3,413 |
| CPU per player-turn | ojh tpm | 6% | 0.0360 s | 0.0595 s | 0.0100 s | 224 |
| Memory | ojh tpm | 8% | 1.11 GiB | same | 2.00 GiB | 1,490 |
| Frame rate | ojh fps | 12% | not measured | n/a | 60 fps | n/a |
| Smoothness | ojh fps | 8% | not measured | n/a | 30 fps | n/a |
| Data per turn | ojh net | 9% | 32.6 KiB | same | 256 KiB | 3,147 |
...
- **Network**: Unciv has no live connection: its multiplayer uploads the whole game after a turn and the other player downloads it. OJH's Unciv driver plays 20 turns with 8 civilizations and reports the size of that compressed game after each one, counted once up and once down; no relay, so turn delivery is n/a
- **Footprint**: install is Unciv.jar (the Java runtime not counted); OJH's Unciv driver plays 20 turns, then times UncivFiles.gameInfoToString with compression, the form Unciv writes saves in, and gameInfoFromString on the result
- **Turn speed settings**: 250 turns asked for, 250 timed, seed 20260914, 8 players chosen by OJH
- **Machine**: Apple M1 Pro, macOS 26.3 (25D125)
- **CPU reference score**: 1,650 rounds/s on one core; CPU-bound figures were put on OJH's reference CPU (1,000 rounds/s) with a factor of 0.606
...
- **Total**: the weighted mean of the parts the results have. Coverage is how much of the weight that was, here 76%; a part a game was not measured for is left out, never counted as zero.
