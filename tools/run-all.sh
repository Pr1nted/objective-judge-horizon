...
if [ -n "$od_server" ] && [ -n "$od_data" ]; then
    measure "Open Doctrines turn speed" tpm opendoctrines --turns "$turns" $common --od-server "$od_server" --od-data "$od_data" --out "$out/opendoctrines-tpm.json"
    ...
    measure "Open Doctrines network" net opendoctrines $common --out "$out/opendoctrines-net.json"
    measure "Open Doctrines frame rate" fps opendoctrines $common --out "$out/opendoctrines-fps.json"
fi

if [ -n "$gd5_python" ] && [ -n "$gd5_dir" ]; then
    measure "Greater Diplomacy 5 turn speed" tpm gd5 --turns "$turns" $common --gd5-python "$gd5_python" --gd5-dir "$gd5_dir" --out "$out/gd5-tpm.json"
    ...
    measure "Greater Diplomacy 5 frame rate" fps gd5 --turns 20 --seconds "$fps_seconds" $common --gd5-python "$gd5_python" --gd5-dir "$gd5_dir" --out "$out/gd5-fps.json"
fi

if command -v "$freeciv_server" >/dev/null 2>&1; then
    measure "Freeciv turn speed" tpm freeciv --turns "$turns" $common --freeciv-server "$freeciv_server" --out "$out/freeciv-tpm.json"
...
if [ -n "$unciv_jar" ]; then
    measure "Unciv turn speed" tpm unciv --turns "$turns" $common --unciv-jar "$unciv_jar" $java_options --out "$out/unciv-tpm.json"
    ...
    measure "Unciv frame rate" fps unciv --turns 20 --seconds "$fps_seconds" $common --unciv-jar "$unciv_jar" $java_options --out "$out/unciv-fps.json"
fi
