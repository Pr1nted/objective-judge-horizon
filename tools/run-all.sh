...
if [ -n "$od_server" ] && [ -n "$od_data" ]; then
    save_option=""
    if [ -n "$od_save" ]; then save_option="--od-save $od_save"; fi
    ...
    measure "Open Doctrines footprint" footprint opendoctrines $common --od-server "$od_server" --od-data "$od_data" $save_option --out "$out/opendoctrines-footprint.json"
    ...
    if [ -n "$od_game" ]; then
        measure "Open Doctrines frame rate" fps opendoctrines --turns 20 --seconds "$fps_seconds" $common --od-game "$od_game" --od-data "$od_data" $save_option --out "$out/opendoctrines-fps.json"
    fi
fi
